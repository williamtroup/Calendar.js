function calendarJs(jd,kd,ld){function U(a){t=h(a)?a:new Date;var b=(new Date(t.getFullYear(),t.getMonth(),1)).getDay();a=nb(t.getFullYear(),t.getMonth());0===b&&(b=7);fa();if(!ob){var c=ta(Ac);c.className="calendar";c.innerHTML="";md(c);nd(c);od(c);pd(c);var e=f("div","row header-days"),g=d.dayHeaderNames.length;c.appendChild(e);for(var k=0;k<g;k++){var l=d.dayHeaderNames[k],m=f("div","cell");m.innerHTML=l;e.appendChild(m)}for(e=0;6>e;e++)for(g=f("div","row"),c.appendChild(g),k=0;7>k;k++)l=7*e+(k+
    ...
    wa.value=d.addText;xa.style.display="none";pb.innerHTML=d.addEventTitle;z={};G.value=Yb(e,G.type);N.value=Zb(e);A.value=Yb(e,A.type);O.value=Zb(e);Ma.checked=!1;ja.value="";Oa.value="";Na.value="";ya.value="#484848";za.value="#F5F5F5";Aa.value="#282828";la.checked=!0;vb.checked=!1;wb.checked=!1;xb.checked=!1;yb.checked=!1;zb.checked=!1;Ab.checked=!1;Bb.checked=!1}qb()}function sd(){var a=Xb(G),b=Xb(A),c=N.value.split(":"),e=O.value.split(":"),g=wc(ja.value),k=wc(Oa.value),l=wc(Na.value);2>c.length?
    $b(d.fromTimeErrorMessage,N):2>e.length?$b(d.toTimeErrorMessage,O):""===g?$b(d.titleErrorMessage,ja):(bd(a,N.value),bd(b,O.value),b<a?$b(d.toSmallerThanFromErrorMessage,A):(cc(),a={from:a,to:b,title:g,description:k,location:l,isAllDay:Ma.checked,color:z.color,colorText:z.colorText,colorBorder:z.colorBorder,repeatEveryExcludeDays:z.repeatEveryExcludeDays},la.checked?a.repeatEvery=0:rb.checked?a.repeatEvery=1:sb.checked?a.repeatEvery=2:tb.checked?a.repeatEvery=3:ub.checked&&(a.repeatEvery=4),h(z.id)?
    Ja.updateEvent(z.id,a,!1):Ja.addEvent(a,!1),ha(),Wb()))}function bd(a,b){var c=0,e=0,g=b.split(":");if(2===g.length){var k=parseInt(g[0]);g=parseInt(g[1]);!isNaN(k)&&2>=k.toString().length&&(c=k);!isNaN(g)&&2>=g.toString().length&&(e=g)}a.setHours(c);a.setMinutes(e)}function Xb(a){var b=new Date;if("date"===a.type)b=new Date(a.value);else if(a=a.value.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/))a=new Date(a[3],a[2]-1,a[1]),a instanceof Date&&!isNaN(a)&&(b=a);return b}function cc(){ia.style.display="none";
